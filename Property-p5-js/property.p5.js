var p5 = require("p5");
//var cv;
function setup() {
	createCanvas(500, 500);
	cv = p5.instance._elements[0].elt;
	cv.style.visibility = "visible";
}
function col(red, green, blue) {
	if(green === undefined && blue === undefined) {
		return {
			r: red,
			g: red,
			b: red
		};
	}else{
		return {
			r: red,
			g: green,
			b: blue
		};
	}
}
function colorToRgb(color) {
	return ("rgb(" + color.r + ", " + color.g + ", " + color.b + ")").toString();
}
function rgbToColor(rgb) {
	var s1 = rgb.replace("rgb(", "");
	var s2 = s1.replace(")", "");
	var sa = s2.split(", ");
	return col(parseInt(sa[0]), parseInt(sa[1]), parseInt(sa[2]));
}

function keypress(p) {
	if(p === typeof Array) {
		for(var i = 0; i < p.length; i++) {
			if(keyIsPressed === true && keyCode == p[i][0]) {
				if(p.t === typeof String) {
					eval(p[i][1]);
				}else{
					p[i][1]();
				}
			}else{
				if(p[i][2] !== undefined) {
					if(p.f === typeof String) {
						eval(p[i][2]);
					}else{
						p[i][2]();
					}
				}
			}
		}
	}else{
		return keyIsPressed && (keyCode == p || key == p);
	}
}

function object(p) {
	if(p.el === undefined) {
		this.p = p;
	    this.x = p.x || 0;
	    this.y = p.y || 0;
	    this.w = p.w || 0;
    	this.h = p.h || 0;
	    this.f = p.f || col(255, 255, 255);
	    this.s = p.s || col(0, 0, 0);
	    this.c = p.c || col(0, 0, 0);
	    this.txt = p.txt || "";
	    this.t = p.t || 1;
	}else{
		this.el = document.getElementById(p.el);
		this.p = p;
		this.x = p.x || parseInt(this.el.style.left);
		this.y = p.y || parseInt(this.el.style.top);
		this.w = p.w || parseInt(this.el.style.width);
		//this.w = p.w || this.el.style.width;
		this.h = p.h || parseInt(this.el.style.height);
		this.f = p.f || rgbToColor(this.el.style.backgroundColor);
		this.s = p.s || rgbToColor(this.el.style.borderColor) || "transparent" || col(255);
		this.c = p.c || col(0) || rgbToColor(this.el.style.color);
		this.txt = p.txt || this.el.textContent;
		this.t = p.t || this.el.style.borderWeight || 1;
	}
}

object.prototype.create = function(asDOMElement) {
	if(this.el === undefined) {
		fill(this.f.r, this.f.g, this.f.b);
		stroke(this.s.r, this.s.g, this.s.b);
		strokeWeight(this.t);
		rect(this.x, this.y, this.w, this.h);
	}else{
		console.log("error");
	}
};
object.prototype.collide = function(p) {
	if(p.s === undefined) {
		p.s = [dist(this.x + this.w/2, 0, p.o.x + p.o.w/2, 0) - (this.w/2 + p.o.w/2), dist(0, this.y + this.h/2, 0, p.o.y + p.o.h/2) - (this.h/2 + p.o.h/2)];
	}
	if(this.r === undefined && p.o.r === undefined) {
		if(((this.x <= p.o.x + p.o.w) && (this.x + this.w >= p.o.x) && (this.y < p.o.y + p.o.h) && (this.y + this.h > p.o.y) && this.x + this.w/2 > p.o.x)) {
			this.x += p.s[0];
			return true;
		}else{return false;}
		if(((this.x <= p.o.x + p.o.w) && (this.x + this.w >= p.o.x) && (this.y < p.o.y + p.o.h) && (this.y + this.h > p.o.y) && this.x + this.w/2 < p.o.x + p.o.w/2)) {
			this.x -= p.s[0];
			return true;
		}else{return false;}
		if(((this.x <= p.o.x + p.o.w) && (this.x + this.w >= p.o.x) && (this.y <= p.o.y + p.o.h) && (this.y + this.h >= p.o.y) && this.y + this.h/2 > p.o.y + p.o.h/2)) {
			this.y += p.s[1];
			return true;
		}else{return false;}
		if(((this.x <= p.o.x + p.o.w) && (this.x + this.w >= p.o.x) && (this.y <= p.o.y + p.o.w) && (this.y +this.h >= p.o.y) && this.y + this.h/2 < p.o.y + p.o.h/2)) {
			this.y -= p.s[1];
			return true;
		}else{return false;}
	}
};
object.prototype.button = function(p) {
	if(p !== undefined) {
		if(this.el === undefined) {
			if(mouseIsPressed && mouseX >= this.x && mouseX <= this.x + this.w && mouseY >= this.y && mouseY <= this.y + this.h) {
				p.t();
			}else{
			    p.f();
			}
		}/*else{
			var input = {d: "", u: ""};
			if(p.os == "desktop" || p.os === undefined) {
				input = {d: "mousedown", u: "mouseup"};
			}else if(p.os == "mobile") {
				input = {d: "touchstart", u: "touchend"};
			}
			this.el.addEventListener(input.d, function() {
				p.t();
			});
			this.el.addEventListener(input.u, function() {
				p.f();
			});
		}*/
	}else{
		if(mouseIsPressed && mouseX >= this.x && mouseX <= this.x + this.w && mouseY >= this.y && mouseY <= this.y + this.h) {
			return true;
		}else{
			return false;
		}
	}
};
object.prototype.mouse = function(p) {
	if(this.r === undefined) {
		switch(p.e) {
			case "click":
				if((mouseIsPressed && mouseX >= this.x && mouseY >= this.y && mouseX <= this.x + this.w && mouseY <= this.y + this.h) && dist(mouseX, mouseY, pmouseX, pmouseY) <= 1) {
					p.t();
				}else{
					p.f();
				}
				break;
			case "drag":
				if((mouseIsPressed && mouseX >= this.x && mouseY >= this.y && mouseX <= this.x + this.w && mouseY <= this.y + this.h) && dist(mouseX, mouseY, pmouseX, pmouseY) >= 2) {
					p.t();
				}else{
					p.f();
				}
				break;
			case "over":
				if(mouseX >= this.x && mouseY >= this.y && mouseX <= this.x + this.w && mouseY <= this.y + this.h) {
					p.t();
				}else{
					p.f();
				}
				break;
			default:
			    break;
		}
	}else{
		switch(p.e) {
			case "click":
				if(mouseIsPressed && dist(mouseX, mouseY, this.x + this.r/2, this.y + this.r/2) <= this.r/2 && dist(mouseX, mouseY, pmouseX, pmouseY) <= 1) {
					p.t();
				}else{
					p.f();
				}
				break;
			case "drag":
				if(mouseIsPressed && dist(mouseX, mouseY, this.x + this.r/2, this.y + this.r/2) <= this.r/2 && dist(mouseX, mouseY, pmouseX, pmouseY) >= 2) {
					p.t();
				}else{
					p.f();
				}
				break;
			case "over":
				if(dist(mouseX, mouseY, this.x + this.r/2, this.y + this.r/2) <= this.r/2) {
					p.t();
				}else{
					p.f();
				}
				break;
			default:
			    break;
		}
	}
};
object.prototype.move = function(p) {
	if(this.x < p.t[0]) {
		this.x += (this.x >= p.t[0]) ? 0 : p.s[0];
	}else if(this.x > p.t[0]) {
		this.x -= (this.x <= p.t[0]) ? 0 : p.s[0];
	}
	if(this.y < p.t[1]) {
		this.y += (this.y >= p.t[1]) ? 0 : p.s[1];
	}else if(this.y > p.t[1]) {
		this.y -= (this.y <= p.t[1]) ? 0 : p.s[1];
	}
};
object.prototype.inherit = function(p) {
	this.x = (p.v[0] == 1) ? p.o.x : (p.v[0] == 2) ? this.x + p.o.x : this.x;
	this.y = (p.v[1] == 1) ? p.o.y : (p.v[1] == 2) ? this.y + p.o.y : this.y;
	this.w = (p.v[2] == 1) ? p.o.w : (p.v[2] == 2) ? this.w + p.o.w : this.w;
	this.h = (p.v[3] == 1) ? p.o.h : (p.v[3] == 2) ? this.h + p.o.h : this.h;
	this.f = (p.v[4] == 1) ? p.o.f : this.f;
	this.s = (p.v[5] == 1) ? p.o.s : this.s;
	this.t = (p.v[6] == 1) ? p.o.t : (p.v[6] == 2) ? this.t + p.o.t : this.t;
	return this;
};
/*
var player = new object({
	x: 100,
	y: 300,
	w: 50,
	h: 50,
	f: col(0, 255, 0),
});

var jump = new object({
	x: 200,
	y: 200,
	w: 50,
	h: 35,
	f: col(150)
});

var m = new object({
	x: 100,
	y: 100,
	w: 50,
	h: 50
});

var ground = new object({
	x: 0,
	y: 350,
	w: 500,
	h: 200,
	f: col(0, 50, 255),
	s: col(0, 0, 200)
});

var b = [];
for(var e = 0; e < 3; e++) {
	b.push(new object({
			x: 500 + (e * 150),
			y: 300,
			w: 50,
			h: 50,
			f: col(0),
			s: col(255)
		})
	);
}

var d = "g";
var su = 9;
var sd = 4;
var ms = 0;
function draw() {
	background(255, 255, 255);
	
	jump.button({
		t: function() {
			jump.f = col(0, 255, 0);
			d = (d == "d") ? "d" : "u";
		},
		f: function() {
			jump.f = col(150);
		}
	});
	m.button({
		t: function() {
			m.f = col(0, 255, 0);
			ms = -5;
		},
		f: function() {
			m.f = col(255);
			ms = 0;
		}
	});
	
	if(d == "u") {
		player.move({
			t: [player.x, 200],
			s: [0, su]
		});
	}
	if(player.y <= 200) {
		d = "d";
	}
	if(d == "d" && !player.collide({o: b[0]})) {
		player.move({
			t: [player.x, 300],
			s: [0, sd]
		});
	}
	if(player.y >= 300) {
		d = "g";
	}
	if(d == "g") {
		player.y = 300;
	}
	
	player.collide({
		o: ground
	})
	
	//player.create();
	jump.create();
	ground.create();
	m.create();
	player.create();
	for(var i = 0; i < 3; i++) {
		if(player.collide({o: b[i]})) {
			player.f = col(255, 0, 0);
		}
		if(!player.collide({o: b[i]})) {
			player.f = col(0, 255, 0) 
		}
		b[0].x += ms;
		b[1].x += ms;
		b[2].x += ms;
	}
	b[0].create();
	b[1].create();
	b[2].create();
	player.x = player.p.x;
	//player.create();
}*/
var player = new object({
	x: 100,
	y: 300,
	w: 25,
	h: 25,
	f: col(0, 255, 0)
});
var ground = new object({
	x: 0,
	y: 405,
	w: 500,
	h: 100,
	f: col(150)
});
var left = new object({
	x: 300,
	y: 150,
	w: 35,
	h: 35,
	f: col(150)
});
var right = new object({
	x: 70,
	f: col(150)
});

var b = [
	new object({
		x: 200,
		y: 380,
		w: 25,
		h: 25
	})
];
for(var i = 1; i < 4; i++) {
	b.push(new object({
		x: 50 * i,
	}).inherit({
		o: b[0],
		v: [2, 1, 1, 1, 1, 1, 1]
	}));
}

right.inherit({
	o: left,
	v: [2, 1, 1, 1, 0, 1, 1]
});

var p = "ground";
player.y = ground.y - player.h;

function draw() {
	background(255, 255, 255);
	
	for(var j = 1; j < 4; j++) {
		b[j].x = (i * 50) + b[j - 1].x;
	}
	
	player.collide({
		o: ground,
		s: [0, 5]
	});
	
	left.button({
		t: function() {
			left.f = col(0, 255, 0);
			b[0].x -= 5;
		},
		f: function() {
			left.f = col(150);
		}
	});
	right.button({
		t: function() {
			right.f = col(0, 255, 0);
			b[0].x += 5;
		},
		f: function() {
			right.f = col(150);
		}
	});
	
	keypress([
		[37, function() {left.f = col(0, 255, 0); b[0].x -= 5;}, function() {left.f = col(150);}],
		[39, function() {right.f = col(0, 255, 0); b[0].x += 5;}, function() {right.f = col(150);}],
	]);
	
	text(keyCode, 10, 10);
	if(mouseIsPressed && p == "ground" && left.button() === false && right.button() === false) {
		p = "up";
	}
	if(p == "up") {
		player.move({
			t: [player.x, 300],
			s: [0, 5]
		});
		text("up", 10, 10);
	}
	if(p == "up" && player.y > 300 && player.y < ground.y - player.h) {
		player.move({
			t: [player.x, 300],
			s: [0, 5]
		});
		text("up", 10, 10);
	}
	if(p == "up" && player.y <= 300) {
		player.move({
			t: [player.x, ground.y - player.h],
			s: [0, 5]
		});
		p = "down";
	}
	if(p == "down" && player.y > 300 && player.y < ground.y - player.h) {
		text("down", 90, 10);
		player.move({
			t: [player.x, ground.y - player.h],
			s: [0, 5]
		});
	}
	if(p == "down" && player.y >= ground.y - player.h) {
		player.y = ground.y - player.h;
		p = "ground";
	}
	
	player.create();
	ground.create();
	left.create();
	right.create();
	for(var k = 0; k < 4; k++) {
		player.collide({
			o: b[k]
		});
		b[k].collide({
			o: player
		});
		
		b[k].create();
	}
}